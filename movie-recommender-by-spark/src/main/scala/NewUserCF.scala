...
object NewUserCF {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val userDataDF = spark.sql("select user_id, movie_id, rating from udata")
    ...
    // 1.2、倒排表(基于item的笛卡儿积)
    val vDataDF = userDataDF.selectExpr("user_id as user_v", "movie_id", "rating as rating_v") // selectExpr可以做一些其他操作，例如改名。。。

    // 根据物品关联，获取评分不一致的.这里的表关系是，movie_id , user_id、user_v 可以理解为倒排表
    val u_v_decare = userDataDF.join(vDataDF, "movie_id")
//      .filter("rating <> rating_v")
      .filter("rating < rating_v")

//    +--------+-------+------+------+--------+
//    |movie_id|user_id|rating|user_v|rating_v|
//    +--------+-------+------+------+--------+
//    |       1|      1|   4.0|   610|     5.0|
//    |       1|      1|   4.0|   609|     3.0|
//    |       1|      1|   4.0|   608|     2.5|
//    |       1|      1|   4.0|   606|     2.5|
    ...
    val df_product = u_v_decare.selectExpr("movie_id", "user_id", "user_v", "rating*rating_v as prod")
    ...
    // 获取user_id物品集合（同时也可以获取user_v的物品集合）。user_id, (user_id_movie_id)
    val df_user_items = userDataDF.rdd.map(row => (row(0).toString, row(1).toString + "_" + row(2).toString))
      .groupByKey()
      .mapValues(_.toArray)
      .toDF("user_id", "item_rating_arr")
/* select * from udata group by user_id */

    val df_user_items_v = df_user_items.selectExpr("user_id as user_v", "item_rating_arr as item_rating_arr_v")
